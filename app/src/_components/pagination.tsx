    ...
    <section className={`max-w-[450px] mx-auto shadow-lg rounded-lg px-5 py-3 ${className}`}>
        ...
        <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
            <div className="flex flex-1 justify-between sm:hidden">
                <button
                value={pagination?.hasPreviousPage ? pagination.previousPage ?? "0" : "0"}
                ...
                <button
                value={pagination?.hasNextPage ? pagination.nextPage ?? "0" : "0"}
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                ...
                <p className="text-sm text-gray-700">
                    Showing <span className="font-medium">{pagination?.previousPage * pagination?.pageSize + 1}</span> to <span className="font-medium">{pagination?.currentPage * pagination?.pageSize}</span> of{' '}
                    <span className="font-medium">{pagination?.totalPages}</span> results
                </p>
                </div>
                <div>
                <nav aria-label="pagination" className="isolate inline-flex -space-x-px rounded-md shadow-xs">
                    ...
                    {Array.from({ length: pagination?.totalPages }, (_, index) => (
                        <button
                            ...
                            value={index + 1}
                            onClick={handlerPagination}
                            className={ `relative inline-flex items-center px-3 py-2 ${ index + 1 === pagination?.currentPage ? currentBtn : defaultBtn }` }
                        >
                            {index + 1}
                        </button>
                    ))}
